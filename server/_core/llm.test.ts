...
describe("LLM boundary", () => {
  ...
  it("validates a completion response and passes an abort signal", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify(validCompletion), { status: 200 })
      );
  ...
  it("rejects malformed, oversized, and schema-invalid completion responses safely", async () => {
    const bodies = [
      ...
      JSON.stringify({
        ...
        choices: [
          {
            index: 0,
            message: { role: "intruder", content: "x" },
            finish_reason: null,
          },
        ],
      }),
    ...
    for (const body of bodies) {
      ...
      await expect(
        invokeLLM({ messages: [{ role: "user", content: "hello" }] })
      ).rejects.toThrow("LLM invoke request failed");
    }
  });
  ...
  it("does not retry permanent client errors or expose provider details", async () => {
    ...
    const response = new Response(
      ...
      { status: 400, statusText: "private" }
    );
    const fetchMock = vi.fn().mockResolvedValue(response);
    vi.stubGlobal("fetch", fetchMock);
    const error = await invokeLLM({
  ...
  it("validates the model list response and rejects malformed entries", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(validModels), { status: 200 })
        )
    );
    ...
    for (const body of [
      JSON.stringify({ data: [{}] }),
      JSON.stringify({ object: "collection", data: validModels.data }),
      JSON.stringify({
        object: "list",
        data: [{ ...validModels.data[0], object: "other" }],
      }),
      JSON.stringify({
        object: "list",
        data: Array.from({ length: 101 }, () => validModels.data[0]),
      }),
    ]) {
      vi.stubGlobal(
        "fetch",
      ...
      await expect(listLLMModels()).rejects.toThrow(
        "List LLM models request failed"
      );
    }
  });

  it("honors a bounded Retry-After for transient rate limits", async () => {
    vi.useFakeTimers();
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
    ...
    const assertion = expect(request).resolves.toEqual(validCompletion);
  ...
  it("lets the overall deadline preempt an excessive Retry-After", async () => {
    vi.useFakeTimers();
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
  ...
  it("rejects oversized request payloads before contacting the provider", async () => {
    ...
    ).rejects.toThrow("LLM invoke request is too large");
