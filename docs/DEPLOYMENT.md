...
The dashboard process validates `PORT` as an integer from 1 through 65535. In production it also requires a server-only `JWT_SECRET` containing at least 32 UTF-8 bytes before listening; missing, blank, or short signing configuration fails startup rather than permitting empty-key session tokens. Generate the value with a cryptographically secure secret manager and never expose it through `VITE_*` or source control. Production binds only the assigned port and exits non-zero with a bounded error-type event if configuration or the initial listen fails; it does not move to an unadvertised port that the hosting router cannot reach. The bounded alternative-port search is development-only.
