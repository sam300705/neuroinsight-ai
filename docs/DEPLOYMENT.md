...
The dashboard was rebuilt after the environment update. A browser-level real-inference test against `https://neuroaiapp-gtbxy6cw.manus.space` passed: one lawful public fixed-split image travelled through the published dashboard, the experimental result rendered, and the protected-save consent control appeared. This checks the live integration only; it does not establish model correctness, clinical validity, or authenticated artifact re-download.
