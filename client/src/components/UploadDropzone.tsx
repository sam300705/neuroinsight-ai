...
export function UploadDropzone({ mode, onSelect }: { mode: AnalysisMode; onSelect: (file: File, check: LocalFileCheck) => void }) {
  const [dragging, setDragging] = useState(false); const inputRef = useRef<HTMLInputElement>(null); const accept = mode === "classification" ? ".png,.jpg,.jpeg,image/png,image/jpeg" : ".nii,.nii.gz,application/x-nifti,application/nifti,application/gzip";
  ...
  return <div onDragOver={event => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); void select(event.dataTransfer.files[0]); }} className={`rounded-2xl border-2 border-dashed p-8 text-center transition ${dragging ? "border-teal-600 bg-teal-50" : "border-slate-300 bg-slate-50"}`}><input ref={inputRef} type="file" accept={accept} className="sr-only" onChange={event => void select(event.target.files?.[0])} /><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-white text-teal-800 shadow-sm">{mode === "classification" ? <Image className="size-6" /> : <ScanLine className="size-6" />}</span><h2 className="mt-4 font-semibold">Drop a supported MRI file here</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-600">{mode === "classification" ? "Mode A accepts PNG or JPEG. The workspace checks extension, MIME type, size, signature, and browser decode before any analysis status is opened." : "Mode B accepts a compatible NIfTI .nii or .nii.gz volume. The workspace checks extension, MIME type, size, and NIfTI/gzip header before server-side metadata verification."}</p><button type="button" onClick={() => inputRef.current?.click()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-teal-800 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-700"><FileUp className="size-4" />Choose file</button><p className="mt-4 flex justify-center gap-2 text-xs text-slate-500"><FileWarning className="size-4" />Content checks are preliminary; full modality and metadata validation remains server-side.</p></div>;
}
